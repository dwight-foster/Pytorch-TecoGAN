...
for n in tqdm(range(num_folders)):
    ...
    for i in tqdm(range(0, num_videos)):
        ...
        if len(images) == 0:
            ...
            num_times = len(images) / args.numframes
            for u in range(int(num_times)):
                h = 0

                for x in range(args.numframes * u, args.numframes * (u + 1)):
                    folder_path = args.OutputDir + "scene_" + str(1000 + z)
                    if not os.path.isdir(folder_path):
                        os.mkdir(folder_path)
                    image = images[x]

                    image_path = folder_path + "/" + "col_high_" + str(h).zfill(4) + ".png"
                    if image is None:
                        pass
                    else:
                        cv2.imwrite(image_path, image)
