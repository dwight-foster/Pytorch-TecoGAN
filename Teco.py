...
def TecoGAN(r_inputs, r_targets, discriminator_F, fnet, generator_F, FLAGS, Global_step, counter1, counter2, optimizer_g
    ...
    gen_flow_lr = fnet(fnet_input.cuda())
    ...
    for frame_i in range(inputimages - 1):
        ...
        inputs = torch.cat((r_inputs[:, frame_i + 1, :, :, :], gen_pre_output_reshape.cpu()), dim=1)
        gen_output = generator_F(inputs.cuda().detach())
    ...
    if (GAN_FLAG):
        ...
        if not FLAGS.pingpang:
            ...
            gen_flow_back_lr = fnet(fnet_input_back.cuda())
        ...
        real_warp0 = F.grid_sample(t_targets, T_vel.cpu())
        ...
        if (FLAGS.Dt_mergeDs):
            ...
            tdiscrim_real_output, real_layers = discriminator_F(real_warp.cuda())
        ...
        if (FLAGS.Dt_mergeDs):
            ...
            fake_warp = torch.cat((before_warp, fake_warp.cpu(), input_hi), dim=1)
    ...
    diff1_mse = s_gen_output.cpu() - s_targets
...
def FRVSR(r_inputs, r_targets, FLAGS, discriminator_F, fnet, generator_F, step, counter1, counter2, optimizer_g, optimizer_d, fnet_optimizer):

    return TecoGAN(r_inputs, r_targets, discriminator_F, fnet, generator_F, FLAGS, step, counter1, counter2, optimizer_g, optimizer_d, fnet_optimizer)
